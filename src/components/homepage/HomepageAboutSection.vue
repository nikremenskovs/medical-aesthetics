<template>
    <section class="max-h-screen mb-12 bg-gradient-to-b from-main-blue/75 to-transparent">
        <div class="px-6 py-12 text-center md:text-left">
            <div class="mx-auto">
                <div class="md:flex md:items-center md:justify-evenly">
                    <div class="my-12 font-yeseva-one text-4xl uppercase tracking-wide md:text-5xl md:w-5/12">
                        <h1 class="mb-2 text-main-white">
                            Iepazisties ar</h1>
                        <h2 class="text-main-blue mb-8">AK Beauty
                            Room</h2>
                        <button
                            class="h-auto w-auto p-4 bg-transparent border-2 border-main-blue rounded-full text-main-blue font-yeseva-one uppercase text-sm font-bold tracking-wider md:text-lg md:tracking-widest hover:bg-main-blue hover:text-main-white ">
                            Uzzinat Vairak
                        </button>
                    </div>
                    <div class="md:w-5/12">
                        <img src="@/assets/images/AnnaEnlarged.jpg" class="rounded-full" />
                    </div>
                </div>
            </div>
        </div>
    </section>
</template>